...
- **Candle colour can differ between exchange feeds.** In the example above the
  ...
  another (green), which decides whether the 3-candle formation exists at all.
  This is not a problem in practice: the bot marks zones on the same feed you
  trade. Verify on that feed rather than against a different one.

  `minBodyPct` is available if you ever want to filter such near-doji candles
  out, but it is **off by default** — the rule as written counts every valid
  formation.
